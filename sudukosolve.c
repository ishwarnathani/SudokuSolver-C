#include <stdio.h>

int puzzle[9][9] = {
    {5, 3, 0, 0, 7, 0, 0, 0, 0},
    {6, 0, 0, 1, 9, 5, 0, 0, 0},
    {0, 9, 8, 0, 0, 0, 0, 6, 0},
    {8, 0, 0, 0, 6, 0, 0, 0, 3},
    {4, 0, 0, 8, 0, 3, 0, 0, 1},
    {7, 0, 0, 0, 2, 0, 0, 0, 6},
    {0, 6, 0, 0, 0, 0, 2, 8, 0},
    {0, 0, 0, 4, 1, 9, 0, 0, 5},
    {0, 0, 0, 0, 8, 0, 0, 7, 9}
};

void printPuzzle(int puzzle[9][9]) {

    printf("\n+-------+-------+-------+");

    for (int i = 0; i < 9; i++) {

        if (i % 3 == 0 && i != 0) {
            printf("\n+-------+-------+-------+");
        }

        printf("\n");

        for (int j = 0; j < 9; j++) {

            if (j % 3 == 0) {
                printf("| ");
            }

            if (puzzle[i][j] == 0) {
                printf("  ");
            } else {
                printf("%d ", puzzle[i][j]);
            }
        }

        printf("|");
    }

    printf("\n+-------+-------+-------+\n");
}

int valid_puzzle(int puzzle[9][9], int row, int col, int num) {

    for (int x = 0; x < 9; x++) {
        if (puzzle[row][x] == num) {
            return 0;
        }
    }

    for (int x = 0; x < 9; x++) {
        if (puzzle[x][col] == num) {
            return 0;
        }
    }

    int startRow = row - row % 3;
    int startCol = col - col % 3;

    for (int i = 0; i < 3; i++) {
        for (int j = 0; j < 3; j++) {
            if (puzzle[i + startRow][j + startCol] == num) {
                return 0;
            }
        }
    }

    return 1;
}

int solve_puzzle(int puzzle[9][9], int row, int col) {

    if (col == 9) {

        if (row == 8) {
            return 1;
        }

        row++;
        col = 0;
    }

    if (puzzle[row][col] > 0) {
        return solve_puzzle(puzzle, row, col + 1);
    }

    for (int i = 1; i <= 9; i++) {

        if (valid_puzzle(puzzle, row, col, i)) {

            puzzle[row][col] = i;

            if (solve_puzzle(puzzle, row, col + 1)) {
                return 1;
            }

            puzzle[row][col] = 0;
        }
    }

    return 0;
}

/*--------------------------------------------------------
  Function: inputPuzzle

  Purpose:
  Accepts Sudoku input from the user.

  Rules:
  - Enter numbers from 0 to 9.
  - 0 represents an empty cell.
  - Checks validity before accepting a number.
--------------------------------------------------------*/

void inputPuzzle(int puzzle[9][9]) {

    printf("\nEnter Sudoku Puzzle\n");
    printf("Use 0 for empty cells.\n\n");

    for (int row = 0; row < 9; row++) {

        for (int col = 0; col < 9; col++) {

            int value;

            while (1) {

                printf("Row %d Column %d : ",
                       row + 1, col + 1);

                scanf("%d", &value);

                if (value < 0 || value > 9) {
                    printf("Invalid input. Enter a value between 0 and 9.\n");
                    continue;
                }

                if (value != 0) {

                    if (!valid_puzzle(puzzle, row, col, value)) {
                        printf("Violates Sudoku rules. Try again.\n");
                        continue;
                    }
                }

                puzzle[row][col] = value;
                break;
            }
        }
    }
}

/*--------------------------------------------------------
  Function: isValidBoard

  Purpose:
  Checks whether the entire Sudoku board satisfies
  Sudoku constraints.

  Returns:
  1 -> Valid board
  0 -> Invalid board
--------------------------------------------------------*/

int isValidBoard(int puzzle[9][9]) {

    for (int row = 0; row < 9; row++) {

        for (int col = 0; col < 9; col++) {

            int current = puzzle[row][col];

            if (current == 0) {
                continue;
            }

            puzzle[row][col] = 0;

            if (!valid_puzzle(puzzle, row, col, current)) {
                puzzle[row][col] = current;
                return 0;
            }

            puzzle[row][col] = current;
        }
    }

    return 1;
}

int main() {

    int choice;

    printf("\n");
    printf("-----------------------------------------------------------------\n");
    printf("                         SUDOKU SOLVER\n");
    printf("-----------------------------------------------------------------\n");
    printf("1. Solve Default Puzzle\n");
    printf("2. Enter Your Own Puzzle\n");
    printf("Enter Choice : ");
    scanf("%d", &choice);

    if (choice == 2) {

        for (int i = 0; i < 9; i++) {
            for (int j = 0; j < 9; j++) {
                puzzle[i][j] = 0;
            }
        }

        inputPuzzle(puzzle);

        if (!isValidBoard(puzzle)) {
            printf("\nInvalid Sudoku Puzzle.\n");
            return 0;
        }
    }

    printf("\nOriginal Puzzle:\n");
    printPuzzle(puzzle);

    if (solve_puzzle(puzzle, 0, 0)) {

        printf("\nSolved Puzzle:\n");
        printPuzzle(puzzle);

    } else {

        printf("\nNo solution exists.\n");
    }

    return 0;
}